...
read_census_data <- function(url){
  ...
  table <- vroom(I(json_result[["result"]]))
}
```
...
```{r readingData}
census <- vroom(here("SAPS2016_SA2017.csv"))
```
...
```{r dataDictionary}
dictionary <- read_xlsx(here("SAPS_2016_Glossary.xlsx"))
```
...
census_save <- function(x, stat_name){
  file_name <- str_c("republic-of-ireland", stat_name, "sa", "2016", sep = "_")
